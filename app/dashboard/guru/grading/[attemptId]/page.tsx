    ...
    return (
        <div className="space-y-8 pb-20 animate-in fade-in duration-700">
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <button onClick={() => router.back()} className="flex items-center gap-2 text-slate-400 hover:text-rose-500 transition-colors font-bold uppercase tracking-widest text-xs mb-4">
                        <ArrowLeft size={16} /> Kembali
                    </button>
                    <h1 className="text-3xl font-black text-rose-600 uppercase italic tracking-tight mb-2">
                        Penilaian Uraian
                    </h1>
                ...
                <button
                    onClick={handleSave}
                    disabled={submitting}
                    className="bg-rose-600 hover:bg-rose-500 text-white font-black px-8 py-4 rounded-2xl flex items-center gap-3 transition-all shadow-sm hover:shadow-lg disabled:opacity-50 tracking-widest text-[10px] uppercase"
                >
                    ...
                    return (
                        <motion.div key={answer.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-8 bg-white border border-slate-100 rounded-[2rem] shadow-sm">
                            <div className="flex justify-between items-start mb-6">
                                <span className="bg-rose-50 text-rose-600 px-3 py-1.5 rounded-lg text-xs font-black tracking-widest">
                                    SOAL {index + 1}
                                </span>
                            ...
                            <div className="flex items-center gap-4 border-t border-slate-100 pt-6">
                                ...
                                <input
                                    ...
                                    className="w-24 border border-slate-200 rounded-xl px-4 py-2 text-center text-lg font-black text-rose-600 outline-none focus:ring-2 focus:ring-rose-500/20"
                                />

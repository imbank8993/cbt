...
import {
    ...
    Upload, Download, FileText, Target, List,
    Clock, ShieldCheck, ArrowLeftCircle,
    FileDown, FileSearch, MoreVertical, Image as ImageIcon, Crop
} from 'lucide-react';
...
export default function BankDetailPage() {
    ...
    // Match Studio: Auto-initialize 4x4 layout for new matching questions
    useEffect(() => {
        if (selectedType === 'matching' && isEditorOpen && options.length === 0) {
            setOptions([
                { text: '', match_texts: [], isCorrect: true, order: 0 },
                { text: '', match_texts: [], isCorrect: true, order: 1 },
                { text: '', match_texts: [], isCorrect: true, order: 2 },
                { text: '', match_texts: [], isCorrect: true, order: 3 },
            ]);
            setMetadata((m: any) => ({
                ...m,
                right_items: ['', '', '', '']
    ...
    }, [selectedType, isEditorOpen, options.length]);

    // Match Studio: Calculate connectors in editor
    useEffect(() => {
        if (selectedType !== 'matching' || !isEditorOpen || !editorMatchingContainerRef.current) {
            setEditorMatchingLinks([]);
            return;
        }
        ...
        const timer = setTimeout(() => {
            ...
            options.forEach((opt, leftIdx) => {
                const leftEl = document.getElementById(`editor-matching-left-${leftIdx}`);
                if (!leftEl || !opt.match_texts) return;

                opt.match_texts.forEach((mText: string) => {
                    const rightEl = document.getElementById(`editor-matching-right-${btoa(mText).replace(/=/g, '')}`);
                    if (!rightEl) return;
    ...
    }, [selectedType, options, metadata.right_items, isEditorOpen, selectedLeft]);
    ...
    const handleOpenView = (q: any) => {
        ...
        // Persist preview state in URL for refresh support
    ...
    const handleExportWord = async (isTemplate = false) => {
            ...
            sections: [{
                children: [
                    new Paragraph({
                        children: [
                            new TextRun({
                                text: isTemplate ? "TEMPLATE IMPORT SOAL - PROKTOR" : `EXPORT SOAL - ${bankInfo?.title || 'BANK SOAL'}`,
                                bold: true,
                                size: 32,
                            }),
                        ],
                    ...
                    new Paragraph({
                        children: [
                            new TextRun({
                                text: isTemplate
                                    ? "Petunjuk: Isi tabel di bawah ini. Jangan menghapus header tabel. Simpan dalam format .docx lalu upload ke menu Import."
                            ...
                            ...(questionsToExport.length > 0 ? questionsToExport.map((q, idx) => {
                                ...
                                if (q.type === 'mcq' || q.type === 'mcq_complex') {
                                    kunci = q.options
                                        ...
                                        .map((o: any) => String.fromCharCode(65 + q.options.indexOf(o)))
                                        .join(', ') || "";
                                } else if (q.type === 'true_false') {
                                    kunci = q.options?.find((o: any) => o.is_correct)?.text || "";
                                } else if (q.type === 'short_answer') {
                                    kunci = q.options?.[0]?.text || "";
                                } else if (q.type === 'matching') {
                                    kunci = q.options?.map((o: any) => `${o.text} == ${o.metadata?.match_text}`).join('\n') || "";
                                } else if (q.type === 'categorization') {
                                    kunci = q.options?.map((o: any) => `${o.text} -> ${o.metadata?.category_name}`).join('\n') || "";
                                }
                                ...
                                return new TableRow({
                                    children: [
                                        ...
                                        new TableCell({ children: q.options?.map((o: any) => new Paragraph(o.text)) || [] }),
                                ...
                                // Contoh 2: PG Kompleks (MCQ_COMPLEX / PGK)
                                new TableRow({
                                    children: [
                                ...
                                // Contoh 3: Benar/Salah (TRUE_FALSE / BS)
                                new TableRow({
                                    children: [
                                ...
                                // Contoh 5: Kategorisasi (CATEGORIZATION / CAT)
                                new TableRow({
                                    children: [
                                ...
                                // Contoh 6: Isian Singkat (SHORT_ANSWER / ISIAN)
                                new TableRow({
                                    children: [
                                ...
                                // Contoh 8: Soal bergambar (MCQ + gambar via tag IMG)
                                new TableRow({
                                    children: [
        ...
        a.href = url;
        a.download = isTemplate ? 'template_soal_proktor.docx' : `export_${bankInfo?.title || 'soal'}.docx`;
        a.click();
        ...
        <div className="min-h-screen bg-slate-50/50 p-4 md:p-8 space-y-8">
            ...
            <header className="flex flex-col gap-8 bg-white p-8 rounded-[2rem] shadow-premium border border-slate-100 relative overflow-hidden">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
                    <div className="flex flex-col gap-3">
                        <Link
                            href="/dashboard/proktor/questions"
                            className="group flex items-center gap-2 text-[#030c4d] font-black text-[9px] uppercase tracking-widest hover:translate-x-[-4px] transition-all"
                        >
                            <ChevronLeft size={14} className="group-hover:text-[#f8a01b] transition-colors" /> Kembali ke Bank Soal
                        </Link>
                        <div>
                        ...
                        <div className="relative" ref={exportRef}>
                            ...
                            <AnimatePresence>
                                {isExportOpen && (
                                    ...
                                    >
                                        <button
                                            onClick={() => { handleExportWord(false); setIsExportOpen(false); }}
            ...
            {isLoading ? (
                <div className="grid grid-cols-1 gap-4">
                    {[1, 2, 3].map((i) => (
                        <div key={i} className="bg-white border border-slate-100 rounded-[1.5rem] p-6 space-y-6 relative overflow-hidden shadow-sm">
                            <div className="flex justify-between items-center">
                                <div className="w-24 h-8 bg-slate-100 rounded-xl animate-pulse" />
                                <div className="flex gap-2">
                                    {[1, 2, 3, 4, 5].map(j => (
                                        <div key={j} className="w-8 h-8 bg-slate-50 rounded-lg animate-pulse" />
                                    ))}
                                </div>
                            </div>
                            <div className="space-y-3 p-4 bg-slate-50/20 rounded-xl border border-slate-50">
                                <div className="w-full h-4 bg-slate-100 rounded-md animate-pulse" />
                                <div className="w-5/6 h-4 bg-slate-100 rounded-md animate-pulse" />
                            ...
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <div className="flex gap-1.5 transition-all">
                                    ...
                                    <button
                                        ...
                                        className="w-8 h-8 bg-white hover:bg-emerald-500 hover:text-white text-slate-400 rounded-lg transition-all shadow-sm border border-slate-100 flex items-center justify-center hover:scale-110 active:scale-90"
                                                            ...
                                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 items-center">
                                                                ...
                                                                <div className="flex items-center gap-4">
                                                                    <div className="flex-shrink-0 flex items-center gap-1.5 text-slate-300">
                                                                        ...
                                                                        <ArrowRight size={14} strokeWidth={3} className={isMatching ? "text-primary animate-pulse-slow" : "text-slate-300"} />
                                                                    </div>
                                                                    <div className="flex-1 flex flex-wrap gap-2">
                                                                        {isMatching ? (
                                                                            (optMeta.match_texts || (optMeta.match_text ? [optMeta.match_text] : [])).map((m: string, mi: number) => (
                                                                                <div key={mi} className="bg-gradient-to-r from-primary/10 to-primary/5 px-4 py-2 rounded-xl text-[10px] font-black text-primary border border-primary/20 uppercase tracking-widest shadow-sm hover:scale-105 transition-transform cursor-default">
                                                                                    {m}
                                                                                </div>
                                                                            ))
                                                                        ) : (
            ...
            <AnimatePresence>
                {
                    isViewModalOpen && (
                        <div className="fixed inset-0 z-[110] flex items-center justify-center p-0 bg-slate-950/60 backdrop-blur-md">
                            <motion.div
                                initial={{ opacity: 0, scale: 0.95 }}
                                animate={{ opacity: 1, scale: 1 }}
                                exit={{ opacity: 0, scale: 0.95 }}
                                className="bg-white w-screen h-screen max-w-none shadow-2xl overflow-hidden relative flex flex-col"
                            >
                                <header className="px-10 py-3 border-b border-slate-100 flex justify-between items-center bg-white/80 backdrop-blur-xl shrink-0">
                                    <div className="flex items-center gap-6">
                                        <div className="w-10 h-10 rounded-xl bg-[#030c4d] flex items-center justify-center text-white shadow-lg shadow-[#030c4d]/10 border border-white/10 font-black text-xl">
                                            P
                                        </div>
                                        <div>
                                            <div className="flex items-center gap-3 mb-0.5">
                                                <h3 className="text-lg font-bold text-[#030c4d] uppercase tracking-tight leading-none">
                                                    Pratinjau
                                                </h3>
                                            </div>
                                            <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-[#f8a01b]">
                                                <span>{bankInfo?.subject || 'MAPEL'}</span>
                                                <span className="text-slate-300">•</span>
                                                <span className="text-slate-500">{bankInfo?.title || 'BANK SOAL'}</span>
                                            </div>
                                        </div>
                                    </div>

                                    <div className="flex items-center gap-4">
                                        <div className="bg-slate-50 border border-slate-300 px-4 py-2 rounded-xl flex items-center gap-3">
                                            <Clock size={14} className="text-[#f8a01b]" />
                                            <span className="font-mono text-base font-black text-[#030c4d]">59:59</span>
                                        </div>
                                        {/* LAYOUT TOGGLE */}
                                        {(() => {
                                        ...
                                        <button
                                            ...
                                            className="w-12 h-12 flex items-center justify-center text-slate-500 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all active:scale-95 border border-transparent hover:border-rose-100"
                                        >
                                            <X size={28} />
                                        </button>
                                    </div>
                                ...
                                <div className="flex-1 overflow-y-auto flex flex-col lg:flex-row gap-8 p-6 lg:px-6 lg:py-8 bg-slate-100/80 relative z-10 max-w-[1700px] mx-auto w-full">
                                        ...
                                        <div className="flex-1 flex flex-col items-center justify-center py-20 text-center space-y-6">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest max-w-xs mx-auto">
                                                    Tidak ada soal yang tersedia untuk dipratinjau.
                                                </p>
                                            </div>
                                        </div>
                                    ) : (
                                        <>
                                            <div className="flex-1 max-w-none mx-auto w-full flex flex-col min-h-0 overflow-hidden">
                                                {(() => {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            {isWide ? (
                                                                ...
                                                                <>
                                                                    {/* LEFT: QUESTION TEXT (50%) */}
                                                                    <div className="flex flex-col min-w-0 h-full overflow-hidden">
                                                                        <div className="bg-white border-2 border-slate-200/60 rounded-[3rem] p-8 lg:p-10 flex-1 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.08)] flex flex-col relative overflow-hidden group">
                                                                    ...
                                                                    <div className="flex flex-col min-w-0 h-full overflow-hidden">
                                                                        <div className="bg-white border-2 border-slate-200/60 rounded-[3rem] p-8 lg:p-10 flex-1 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.08)] flex flex-col relative overflow-hidden group">
                                                                            <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-[#f8a01b]/5 blur-[100px] rounded-full -z-10" />
                                                            ...
                                                            <div className="flex flex-col gap-6 h-full overflow-hidden">
                                                                <div className="bg-white border-2 border-slate-200/60 rounded-[2.5rem] p-6 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.08)] relative overflow-hidden flex flex-col h-full shrink-0">
                                                                    ...
                                                                    <div className="flex items-center justify-between mb-6 shrink-0">
                                                                        <div>
                                                                            ...
                                                                            <p className="text-[8px] font-bold text-slate-300 uppercase tracking-widest">Preview Mode</p>
                                                                        </div>
                                                                    </div>

                                                                    <div className="grid grid-cols-4 gap-1.5 mb-6 overflow-y-auto pr-1 custom-scrollbar shrink min-h-0">
                                                                    ...
                                                                    <div className="mt-auto pt-4 border-t border-slate-100 space-y-3 shrink-0">
                                                                        {/* Toggle Ragu-ragu (Current Question) */}
                                                                        <button
                                                                            onClick={() => {
                                                                            ...
                                                                            className={`w-full py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 group active:scale-95 border-2 ${doubtfulPreviews.has(displayQ.id)
                                                                                ? 'bg-[#f8a01b] border-[#f8a01b] text-white shadow-lg shadow-[#f8a01b]/20'
                                                                                : 'bg-white border-slate-100 text-slate-500 hover:border-[#f8a01b]/30 hover:bg-[#f8a01b]/5'
                                                                                }`}
                                                                        >
                                                                            <Check size={16} strokeWidth={3} className={doubtfulPreviews.has(displayQ.id) ? "text-white" : "text-transparent"} />
                                                                            Ragu-Ragu
                                                                        </button>
                                                                        ...
                                                                        {!isSimulating ? (
                                                                            <button
                                                                                ...
                                                                                disabled={Object.keys(matchingAnswers).length === 0}
                                                                                className="w-full bg-[#f8a01b] hover:bg-[#e08e15] text-white py-3 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-[#f8a01b]/20 transition-all flex items-center justify-center gap-2 group active:scale-95 disabled:opacity-30 disabled:grayscale"
                                                                            >
                                                                            ...
                                                                            <button
                                                                                ...
                                                                                className="w-full bg-slate-900 hover:bg-black text-white py-3 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-black/20 transition-all flex items-center justify-center gap-2 group active:scale-95"
                                                                            >
            ...
            <ImageCropperModal
                ...
                onCropComplete={async (croppedData) => {
                    ...
                    // 1. Upload to Hosting
                    const file = dataURLtoFile(croppedData, `crop_${Date.now()}.png`);
                    const uploadResult = await uploadToHosting(file, 'questions');

                    if (!uploadResult.success || !uploadResult.url) {
                        alert('Gagal mengunggah foto ke server: ' + uploadResult.error);
                        setIsCropperOpen(false);
                        return;
                    }

                    const finalImgUrl = uploadResult.url;

                    // 2. Insert URL to Editor/Option
                    if (cropperContext.type === 'question') {
                        if (quillInstance) {
                            const range = quillInstance.getSelection();
                            quillInstance.insertEmbed(range?.index || 0, 'image', finalImgUrl);
                        }
                    } else if (cropperContext.type === 'option' && cropperContext.index !== undefined) {
                        const imgHtml = `<img src="${finalImgUrl}" style="max-width:100%; height:auto;" />`;
